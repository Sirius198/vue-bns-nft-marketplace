...
<template>
  ...
  <footer v-if="states?.status === WalletStatus.WALLET_NOT_CONNECTED">
    <button v-for="(connectType, index) in availableInstallTypes" :key="index"
      v-on:click="controller.install(connectType)">
      Install {{ connectType }}
    </button>
    <button v-for="(connectType, index) in availableConnectTypes" :key="index"
      v-on:click="controller.connect(connectType)">
      Connect {{ connectType }}
    </button>
    <br />
    <button v-for="(connection, index) in availableConnections" :key="index"
      v-on:click="controller.connect(connection.type, connection.identifier)">
      <img v-bind:src="connection.icon" v-bind:alt="connection.name" style="width: 1em; height: 1em" />
